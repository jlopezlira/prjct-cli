...
/**
 * Shell command for a Claude hook — the shared native → direct → portable
 * chain (see core/services/hook-command.ts). Claude needs no host env
 * prefix: it is the default host when PRJCT_HOOK_HOST is unset.
 */
function hookCommand(subcommand: string): string {
  return hookCommandChain(subcommand)
}

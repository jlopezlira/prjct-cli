...
export function buildPrjctSkillReference(): string {
  return [
    ...
    '3. **Follow the card**: `prjct qa next` says what is left. On `dispatch_qa_agent`, run `prjct qa brief --md` and hand it as the ONLY input to a fresh `general-purpose` subagent — never your transcript or diff (information asymmetry, like the contradictory review). It drives the pending flows with its browser tool (Playwright MCP, Chrome DevTools MCP, agent-browser, Claude in Chrome — or `curl` for API paths) and files `prjct qa report --json \'[{"id","verdict","evidence"}]\'`. Your own `prjct qa mark` is `author` evidence and never satisfies strict.',
    ...
    '',
    'The audit is an orchestrator — it does the heavy work via subagents, not itself. It collects the diff scope (`git diff <base>...HEAD --name-only --stat`; if empty, abort with "Nothing to audit on this branch") and dispatches THREE subagents IN PARALLEL via the Agent tool (one tool-use block each, SAME message), each inheriting the session model (do not set `model:`):',
